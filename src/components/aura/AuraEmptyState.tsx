...
export function AuraEmptyState({ firstName }: { firstName?: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-12 text-center">
      <span className="grid h-16 w-16 place-items-center rounded-2xl border border-primary/30 bg-primary/10 text-primary xl:h-[72px] xl:w-[72px]">
        <Sparkles className="h-8 w-8 xl:h-9 xl:w-9" aria-hidden="true" />
      </span>
      <h2 className="font-display text-xl font-semibold text-foreground xl:text-2xl">
        {firstName ? `Hello, ${firstName}. I'm Aura.` : "Hello. I'm Aura."}
      </h2>
      <p className="max-w-sm text-sm text-muted-foreground xl:text-base">
