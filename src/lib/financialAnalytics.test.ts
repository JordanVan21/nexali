...
describe("sumIncomeExpense", () => {
  it("sums income and expenses within the range only, excluding income from expenses", () => {
    ...
    const transactions: TransactionWithCat[] = [
      ...
      tx({ amount: 9999, created_at: "2025-05-15T12:00:00Z" }), // outside range
    ];
...
describe("buildMonthlyBuckets", () => {
  it("builds the requested number of buckets, oldest to newest, labeled by month", () => {
    const transactions: TransactionWithCat[] = [
      tx({ amount: 500, created_at: "2025-06-02T12:00:00Z", categories: { id: 9, name: "Salary", type: "income" } }),
      tx({ amount: 100, created_at: "2025-05-02T12:00:00Z" }),
    ];
...
describe("expenseCategoryTotals", () => {
  ...
  it("groups real expense transactions by category with amount, count, and percent of the range total", () => {
    const transactions: TransactionWithCat[] = [
      ...
      tx({ amount: 100, created_at: "2025-06-03T12:00:00Z", category_id: 2, categories: { id: 2, name: "Dining", type: "expense" } }),
    ];
  ...
  it("excludes income transactions from the spending breakdown", () => {
    const transactions: TransactionWithCat[] = [
      tx({ amount: 5000, created_at: "2025-06-01T12:00:00Z", categories: { id: 9, name: "Salary", type: "income" } }),
      tx({ amount: 100, created_at: "2025-06-02T12:00:00Z" }),
    ];
  ...
  it("breaks ties deterministically by category name", () => {
    const transactions: TransactionWithCat[] = [
      tx({ amount: 100, created_at: "2025-06-01T12:00:00Z", category_id: 2, categories: { id: 2, name: "Zebra", type: "expense" } }),
      tx({ amount: 100, created_at: "2025-06-01T12:00:00Z", category_id: 1, categories: { id: 1, name: "Apple", type: "expense" } }),
    ];
  ...
  it("respects topN after sorting by amount descending", () => {
    const transactions: TransactionWithCat[] = [
      tx({ amount: 300, created_at: "2025-06-01T12:00:00Z", category_id: 1, categories: { id: 1, name: "Housing", type: "expense" } }),
      tx({ amount: 200, created_at: "2025-06-02T12:00:00Z", category_id: 2, categories: { id: 2, name: "Dining", type: "expense" } }),
      tx({ amount: 100, created_at: "2025-06-03T12:00:00Z", category_id: 3, categories: { id: 3, name: "Fuel", type: "expense" } }),
    ];
